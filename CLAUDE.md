...
| Web UI | login, browse, upload and download files today; the calendar later | any browser |
...
- A web UI at `/`: sign in, walk the tree, download a file, upload one, make a
  folder. The upload streams into the blob store and replaces like a PUT does,
  and nothing can be renamed or deleted from there yet. The session is
...
Not written yet: CalDAV, the rest of the web UI -- renaming, deleting and the
